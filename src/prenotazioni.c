...
Prenotazione crea_prenotazione_test(int id_utente, int id_veicolo, int giorno_inizio, int ora_inizio,
                                  ...
                                  int id_prenotazione) {
    // Validazione dei parametri base
    ...
    list veicoli = get_lista_veicoli_test();  // Usa la lista veicoli di test
    ...
    if (valida_data_prenotazione(timestamp_inizio, timestamp_fine) != 0) {
        return NULL;
    }

    // Verifica sovrapposizioni
    if (verifica_sovrapposizioni(get_coda_test(), id_veicolo, timestamp_inizio, timestamp_fine) != 0) {
        return NULL;
    }
    ...
    nuova->id_utente = id_utente;
    nuova->id_veicolo = id_veicolo;
    nuova->giorno_ora_inizio = timestamp_inizio;
    nuova->giorno_ora_fine = timestamp_fine;
